...
- But, 정규화해서 오히려 성능이 나빠질 수 있다 (ex. 조인 오버헤드)
  - 필요시 비정규화
## ■ 정규형 원칙
...
<img width="530" height="397" alt="image" src="https://github.com/user-attachments/assets/2003f1ac-02ed-4903-979e-f107553241a2" />

- **기본키**: (`유저ID`, `수강명`) → 복합키
- **문제점**: `유저번호`는 `유저ID`에만 종속됨 → **부분 함수 종속 발생**
- **부분 함수 종속 제거**: `유저번호`를 다른 테이블로 분리
## ■ 제3정규형
<img width="456" height="397" alt="image" src="https://github.com/user-attachments/assets/57a87820-2e6a-4c87-b229-9a34d23a9086" />

- 2NF 만족 + 이행적 함수 종속 제거
- 비프라임 속성(`할인율`)이, 다른 비프라임 속성(`등급`)에 의존하지 않게 함
### ▶ 이행적 함수 종속
...
- <img width="327" height="244" alt="image" src="https://github.com/user-attachments/assets/f8b1281e-dd64-411a-b83e-52d4358591d3" />
- <img width="265" height="179" alt="image" src="https://github.com/user-attachments/assets/f247d9c2-bc40-4d88-b167-381f84574c62" />
- **후보키** : `(학번, 수강명)` `(학번, 강사)`
- **결정자** : `(학번, 수강명)`, `강사`
- 여기서 `강사`는 후보키가 아니지만, `수강명`은 **후보키의 일부**이므로 **3NF를 만족한다**
- 그러나 `강사`가 후보키가 아니므로 **BCNF는 만족하지 않는다**
- 따라서 이상현상이 생긴다
  - 삽입 이상 : 범석이 롤을 담당할 때, 수강생이 없으면 NULL
  - 삭제 이상 : 12101이 MEVN을 취소하면
  - 갱신 이상
- <img width="464" height="479" alt="image" src="https://github.com/user-attachments/assets/031e964a-3012-431f-b87e-fada52564d73" />
- 이렇게하면 BCNF도 만족한다
### ▶ 후보키가 아님 vs 비프라임
| 구분                                | 정의                       | 예시 (후보키: **A,B**) | A | B | C |
| --------------------------------- | ------------------------ | ----------------- | - | - | - |
| **후보키 (Candidate Key)**           | 유일성 + 최소성을 모두 만족하는 속성 집합 | (A,B)             | ❌ | ❌ | ❌ |
| **프라임 속성 (Prime Attribute)**      | 후보키에 속하는 속성              | (A,B)에 속함         | ✅ | ✅ | ❌ |
| **비프라임 속성 (Non-Prime Attribute)** | 어떤 후보키에도 속하지 않는 속성       | C                 | ❌ | ❌ | ✅ |

- 후보키가 복합키인 경우, 각각의 속성은 개별적으로는 후보키는 아니지만 주속성(프라임 키)이라고 부른다 
