...
#### MAC주소
> LAN 카드(NIC)에 하드웨어적으로 부여된 식별번호
- 48bit = 6Byte
- 전 세계적으로 유일하도록 제조사 단위에서 관리된다.
## 계층 간 데이터 송수신 과정
<img width="585" height="446" alt="image" src="https://github.com/user-attachments/assets/de44ac81-a919-4011-8fe7-f1efe199e01c" />

> 송신측의 캡슐화와 수신축의 비캡슐화
### 캡슐화 과정
<img width="772" height="290" alt="image" src="https://github.com/user-attachments/assets/57a24b23-e0f2-4c36-8f91-a8d1dbac07ae" />

> 상위 계층에서 내려온 데이터를 하위 계층이 자신의 헤더를 덧붙여 감싸는 과정
- 비유 : 편지(데이터) → 봉투(TCP 헤더) → 상자(IP 헤더) → 포장 박스(프레임 헤더) → 택배 트럭(물리 신호)
### 비캡슐화 과정
<img width="622" height="242" alt="image" src="https://github.com/user-attachments/assets/5358a418-3967-4fd4-b50b-144cd950c0be" />

> 하위 계층에서 올라오며 각 계층의 헤더를 제거해 원본 데이터를 복원하는 과정
# PDU(Protocol Data Unit)
> 특정 계층에서 데이터를 주고받을 때, 그 계층 고유의 데이터 단위
- PDU = 헤더 + 페이로드
  - **헤더(Header)** – 제어 정보
    - 목적지 식별
    - 오류 제어
    - 흐름 제어
    - 포트 번호, IP 주소, MAC 주소, 체크섬 등
  - **페이로드(Payload)** – 실제 전달할 데이터

| 계층            | PDU 명칭                         | 포함하는 헤더 정보                        |
| ------------- | ------------------------------ | --------------------------------- |
| **애플리케이션 계층** | **메시지(Message)**               | HTTP 헤더, SMTP 헤더 등                |
| **전송 계층**     | **세그먼트(TCP)** / **데이터그램(UDP)** | 출발지·목적지 포트, 순서번호, 체크섬             |
| **인터넷 계층**    | **패킷(Packet)**                 | 출발지·목적지 IP 주소, TTL, 프로토콜(TCP/UDP) |
| **링크 계층**     | **프레임(Frame)**                 | 출발지·목적지 MAC 주소, EtherType, CRC    |
| **물리 계층**     | **비트(Bit)**                    | 전기/광/전파 신호로 변환된 데이터 흐름            |
